...
stepwise<-function(full.model, initial.model, alpha.to.enter, alpha.to.leave,data) {
      ...
      # (Useful things for someone to add: a list of variables that must be included)
      env<-environment()
      environment(full.model) <- env
      environment(initial.model) <-env  #this is what allows a data frame to be passed!
